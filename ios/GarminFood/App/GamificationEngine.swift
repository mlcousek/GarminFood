...
final class GamificationEngine {
    ...
    func refresh(now: Date = Date()) async {
        ...
        streakStatus = StreakEngine.status(events: events, now: now)
        ...
        if let template = ChallengeCatalog.all.first(where: { $0.id == active.templateId }),
           ChallengeEngine.isWindowElapsed(active: active, template: template, now: now) {
            _ = try? await challengeStore.rotateIfWindowElapsed(catalog: ChallengeCatalog.all, now: now, baselineStreakLength: streakStatus.length)
        }
    ...
    func handleLogConfirmed(now: Date = Date()) async {
        let previousStreak = StreakEngine.status(events: lastKnownEvents, now: now)
        let events = await usageHistory.all() // already includes the entry that was just confirmed
        let newStreak = StreakEngine.status(events: events, now: now)
        let streakExtended = newStreak.length > previousStreak.length

        let goalStatuses = await goalStatusStore.all()
        let today = NutritionDayBoundary.dayString(for: now)
    ...
    private func refreshChallengeDisplay(events: [UsageEvent], goalStatuses: [DailyGoalStatus], now: Date) async {
        ...
        challengeProgress = ChallengeEngine.progress(for: template, active: active, events: events, goalStatuses: goalStatuses, now: now)
    }
    ...
    private func checkChallengeCompletion(events: [UsageEvent], goalStatuses: [DailyGoalStatus], now: Date) async {
        ...
        let progress = ChallengeEngine.progress(for: template, active: active, events: events, goalStatuses: goalStatuses, now: now)
        ...
        pendingMoments.append(.challengeCompleted(title: template.title, xpAwarded: xpResult?.xpAwarded ?? template.xpReward))
        ...
        if let rotated, let newTemplate = ChallengeCatalog.all.first(where: { $0.id == rotated.templateId }) {
            ...
            challengeProgress = ChallengeEngine.progress(for: newTemplate, active: rotated, events: events, goalStatuses: goalStatuses, now: now)
        }
    }
