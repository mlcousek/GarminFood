...
- [~] 6.4 Create a throwaway app + widget extension pair (still via the CI pipeline from 6.1-6.3, not local Xcode). Add the Keychain Sharing capability (`keychain-access-groups` entitlement) to both targets with a common group name. Write an item from the app, read it from the extension. Record the result — this decides whether D3's shared-token design or its degraded fallback is what gets built. **CI half done and verified 2026-09-14**: `GarminFoodWidgetExtension` scaffolded, build succeeded, and the resulting `.ipa` was inspected by hand — `Payload/GarminFood.app/PlugIns/GarminFoodWidgetExtension.appex/` is correctly embedded. **Still open**: the actual on-device result (does AltStore's free-tier signing grant the custom `keychain-access-groups` entitlement at all) needs the owner to reinstall via AltStore and add the "Keychain Spike" widget to their Home Screen — cannot be observed from this environment.
