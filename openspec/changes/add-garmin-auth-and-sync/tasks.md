...
- [ ] 6.3 Download the unsigned `.ipa` artifact from the Actions run and sign+install it onto the iPhone using **AltServer for Windows + AltStore on the device** — the same tool already committed to in D8 for the ongoing 7-day resign cycle, so no second tool is needed for the long haul. One-time: AltServer installs the AltStore app onto the phone (Apple ID entered directly into AltServer, never into CI or a GitHub secret); after that, AltStore on the device installs the custom `.ipa` (via the Files app), and AltServer auto-refreshes it every 7 days over Wi-Fi. Sideloadly or iSign Loader remain documented fallbacks if AltServer/AltStore gives trouble. Confirm the app installs and opens. (Requires the owner's phone — cannot be done from this environment.)
