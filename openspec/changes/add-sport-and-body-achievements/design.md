...
| `sport.double-day` | Double Day | two counted activities on one day and the protein goal met that day | uncommon |
...
| `body.fast-3` / `-7` / `-14` / `-30` | Fasting streak badges | `FastingDayEvaluator.keptStreak` reaches 3 / 7 / 14 / 30 | common / uncommon / rare / epic |
...
gain). Without a start weight, only `body.target` and `body.steady-30`
apply. Badges are permanent; a later goal change does not re-arm them.
...
`features/sport/sport.json`: `{ fuelledActivityIds: [String],
recoveredActivityIds: [String], earnedDays: [String], raceDays: [String] }`,
each capped at 2,000, Optional fields, quarantine helpers. Unlock state
lives in `AchievementStore`.
...
("Fuelled right — 45 g carbs 90 min before your run") fires at most once
per activity, the first time it counts, style `.celebration`, 0 XP.

### D6 — UI

- `SportBodySlotView`: "This month: 6 runs fuelled · 4 recovered", weight
